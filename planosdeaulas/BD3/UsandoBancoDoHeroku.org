...
* Procedimento didatico
  ...
  + Demonstracao pratica do professor
  + Atividade pratica com apoio do professor

* Exercicioos e Atividades de Reflexão/Fixação
  Implementacao de um banco de dados usando postgresql no heroku

* Avaliação
  Por enquanto nenhuma

* Introducao
  Parece um pouco dificil esse assunto, essa aula, mas se vc ler com
...
#+BEGIN_SRC shell :session s1 :results output :exports both
  backupsys::DATABASE=> \i /home/wagner/fzlbpms/submodules/somewritings/planosdeaulas/BD3/sql_pgscripts/scriptdeteste.sql 
CREATE TABLE
backupsys::DATABASE=> \d
               Lista de relações
 Esquema |    Nome    |  Tipo  |      Dono      
